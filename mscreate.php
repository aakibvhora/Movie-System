...
if (isset($_POST['register']))
{
    ...
    else
    {
        ...
        addUser($userLogin, $userPassword, $firstName, $lastName, $address, $city, $state, $zip, $country, $eMail, $phone);
...
<form name ="addUserForm" class="form-horizontal" role="form" id="addUserForm" action="mscreate.php" method="post">
    <div class="form-group">
    <label for="userlogin" class="col-sm-5">Username:</label>
    <div class="col-sm-4">
   <input type="text" name="userlogin" id ="userlogin" value="<?php echo $userLogin; ?>" class="form-control" maxlength="10" autofocus="autofocus" required="required" pattern="^[\w@\.-]+$" title="Valid characters are a-z 0-9 _ . @ -" autofocus/><br />
   </div>
    </div>
   <div class="form-group">
    <label for="userpassword" class="col-sm-5">Password:</label> 
   <div class="col-sm-4">
       <input type="password" name="userpassword" autocomplete="new-password" id="userpassword" value="<?php echo $userPassword; ?>" class="form-control" maxlength="10" required="required" pattern="^[\w@\.-]+$" title="Valid characters are a-z 0-9 _ . @ -" /><br />
   </div>
        </div>
    <div class="form-group">
   ...
   <div class="col-sm-4">
       <input type="email" name="email" id ="email" value="<?php echo $eMail; ?>" maxlength="50" class="form-control" required="required" pattern="^([\w-\.]+)@([\w]+)\.([a-zA-Z]{2,4})$" title="Enter a valid email" /> <br />
   </div></div>
   <div class="form-group">
   <label for="phone" class="col-sm-5">Telephone:</label>
   <div class="col-sm-4">
       <input type="text" name="phone" id ="phone" value="<?php echo $phone; ?>" maxlength="12" class="form-control" required="required" pattern="^(\d{3}-)?\d{3}-\d{4}$" title="Enter a valid phone number For Ex:970-825-3395" /><br />
   </div>
   </div>
    <!--<div class="col-sm-7 col-sm-offset-5">-->
   <div class="form-group">
    <div class="row">
      <input type="submit" class="col-sm-offset-4 col-sm-2 btn btn-primary" value="Create" name="register" /> 
